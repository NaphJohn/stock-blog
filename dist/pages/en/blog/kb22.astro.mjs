import { c as createComponent, r as renderComponent, a as renderTemplate, u as unescapeHTML } from '../../../chunks/astro/server_CYDbQRlG.mjs';
import 'piccolore';
import { $ as $$BlogPost } from '../../../chunks/BlogPost_DVC32pQ4.mjs';
export { renderers } from '../../../renderers.mjs';

const html = () => "<blockquote>\n<p>This is Lesson 22/31 of <em>Stock Knowledge 101</em> 〔Financials〕.</p>\n</blockquote>\n<ul>\n<li>Left = assets (where money went): cash, receivables, inventory, fixed assets, goodwill.</li>\n<li>Right = liabilities + equity (where money came from): debt, payables, retained earnings.</li>\n<li>Asset-liability ratio = total liabilities/total assets; >70% = heavy repayment pressure (varies by industry).</li>\n<li>Beware ‘high cash + high debt’ and huge goodwill (impairment landmine).</li>\n</ul>\n<blockquote>\n<p>📌 Three things: is leverage too high, is cash real, is goodwill a mine.</p>\n</blockquote>\n<blockquote>\n<p>⚠️ This article is a methodological illustration and does not constitute any investment or trading advice. Markets are risky; decide with caution.</p>\n</blockquote>";

				const frontmatter = {"title":"Reading the balance sheet","description":"Financials · Left = assets (where money went): cash, ","layout":"../../../layouts/BlogPost.astro","series":"Stock Knowledge 101","free":true,"lang":"en","altLang":"中文","altHref":"/blog/kb22","pubDate":"2026-08-11T00:00:00.000Z"};
				const file = "/Users/hangkai/WorkBuddy/Claw/_stock_blog_src/src/pages/en/blog/kb22.md";
				const url = "/stock-blog/en/blog/kb22";
				function rawContent() {
					return "   \n                                  \n                                                                    \n                                       \n                           \n          \n        \n           \n                   \n                   \n   \n>This is Lesson 22/31 of *Stock Knowledge 101* 〔Financials〕.\n\n- Left = assets (where money went): cash, receivables, inventory, fixed assets, goodwill.\n- Right = liabilities + equity (where money came from): debt, payables, retained earnings.\n- Asset-liability ratio = total liabilities/total assets; >70% = heavy repayment pressure (varies by industry).\n- Beware 'high cash + high debt' and huge goodwill (impairment landmine).\n\n\n> 📌 Three things: is leverage too high, is cash real, is goodwill a mine.\n\n> ⚠️ This article is a methodological illustration and does not constitute any investment or trading advice. Markets are risky; decide with caution.\n";
				}
				async function compiledContent() {
					return await html();
				}
				function getHeadings() {
					return [];
				}

				const Content = createComponent((result, _props, slots) => {
					const { layout, ...content } = frontmatter;
					content.file = file;
					content.url = url;

					return renderTemplate`${renderComponent(result, 'Layout', $$BlogPost, {
								file,
								url,
								content,
								frontmatter: content,
								headings: getHeadings(),
								rawContent,
								compiledContent,
								'server:root': true,
							}, {
								'default': () => renderTemplate`${unescapeHTML(html())}`
							})}`;
				});

const _page = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
	__proto__: null,
	Content,
	compiledContent,
	default: Content,
	file,
	frontmatter,
	getHeadings,
	rawContent,
	url
}, Symbol.toStringTag, { value: 'Module' }));

const page = () => _page;

export { page };
